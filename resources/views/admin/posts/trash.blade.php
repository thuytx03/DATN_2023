                ...
                <div class="table-responsive">
                    <div id="dataTable_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
                        <div class="row">
                            ...
                            <div class="col-sm-12 col-md-6">
                                <form action="{{ route('post.trash') }}" method="get">
                                    <div class="row">
                           ...
                           <th>hành động</th>
                            ...
                            @foreach($trashedPosts as $post)
                                ...
                                <td class="sorting_1 text-center">
                                            <label>
                                                <input type="checkbox" class="child-checkbox"  value="{{$post->id}}" name="ids[]">
                                        </label>
                                </td>
                                    <td name="id" class="id">{{ $post->id }}</td>
                                    <td name="title" id="title">{{ $post->title }}</td>
                                    <td>{{ $post->slug }}</td>
                                    
                                    <td>
                                        <a href="{{ route('post.restore', $post->id) }}" class="btn btn-success">Khôi phục</a>
                                        <a href="{{ route('post.forceDelete', ['id' => $post->id]) }}" class="btn btn-danger
                                        ...
                                        <form action="{{ route('post.forceDelete', $post->id) }}" method="post" style="display:inline;" id="delete" >
                                            @csrf
                                        </form>
        ...
        function updateStatus() {
            ...
            $(document).ready(function() {
                $('.switch-status').change(function() {
                    ...
                    $.ajax({
                        method: 'POST',
                        url: 'admin/post/update-status/' + itemId,
                        data: {
                            _token: '{{ csrf_token() }}',
                            status: status
                        },
                    ...
                    if (selectedCheckboxes.length > 0) {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                ...
                                $.ajax({
                                    type: 'POST',
                                    url: '/admin/post/permanentlyDeleteSelected', // Thay thế bằng tuyến đường xử lý xoá của bạn
                                    data: {
                                        ids: selectedIds,
                                        _token: '{{ csrf_token() }}',
                                    },
