            ...
            <form method="post" action="{{route('post.update',$data->id)}}" enctype="multipart/form-data">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input target="_blank" class="form-control" name="slug" id="slug" value="{{$data->slug}}">
                        </div>
                        <div class="mb-3">
                                ...
                                <ul id="div1">
                                    <li>
                                        ...
                                        @if(in_array($post_type1->id,$post_type_posts->pluck('post_type')->toArray()))
                                            checked @endif 
                                        >
                                        <label for="post_type{{ $post_type1->id }}">{{ $post_type1->name }}</label>
                                        <ul>
                                            @foreach ($post_type as $child)
                                                @if ($child->parent_id == $post_type1->id)
                                                    <li>
                                                        <input type="checkbox" id="post_type{{ $child->id }}" name="post_type[]" value="{{ $child->id }}"
                                                        @if(in_array($child->id,$post_type_posts->pluck('post_type')->toArray()))
                                                    checked @endif >
                                                        <label for="post_type{{ $child->id }}">{{ $child->name }}</label>
                                                    </li>
                                                @endif
                                            @endforeach
                                        </ul>
                ...
                <div class="">
                    <div class="row_content ">
                        <textarea name="content" id="contents" cols="30" rows="10" width="100%" value="{{$data->content}}" style="width: 100%; overflow-y: scroll;">
                        {{$data->content}}
                        </textarea>
