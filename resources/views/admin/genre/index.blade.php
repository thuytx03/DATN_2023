                    ...
                    <div class="col-md-6 text-right">
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                <a class="dropdown-item" href="{{ route('genre.trash') }}">Thùng rác</a>
                                ...
                                <table class="table table-bordered dataTable no-footer" id="dataTable" width="100%"
                                    ...
                                    @foreach($genres as $genre)
                                        <tr class="odd">
                                            ...
                                            <td class="text-center">
                                                <input type="checkbox" value="{{ $genre->status }}"
                                                       name="status" data-item-id="{{ $genre->id }}"
                                                       class="switch1 switch-status switchery-small"
                                                    {{ $genre->status == 1 ? 'checked' : '' }}/>
                                            </td>
                                            <td class="text-center">
                                                <div class="dropdown">
        ...
        function updateStatus() {
            $(document).ready(function () {
                $('.switch-status').change(function () {
                    ...
                    $.ajax({
                        method: 'POST',
                        url: '/admin/genre/update-status/' + itemId,
                        data: {
                            _token: '{{ csrf_token() }}',
                            status: status
                        },
                    ...
                    if (selectedCheckboxes.length > 0) {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                ...
                                $.ajax({
                                    type: 'POST',
                                    url: '/admin/genre/deleteAll', // Thay thế bằng tuyến đường xử lý xoá của bạn
                                    data: {
                                        ids: selectedIds,
                                        _token: '{{ csrf_token() }}',
                                    },
