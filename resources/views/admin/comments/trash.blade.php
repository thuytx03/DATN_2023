                    ...
                    <div id="dataTable_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
                       ...
                       <td class="pr-2 " tabindex="0" aria-controls="dataTable"
                           ...
                           <th>hành động</th>
                            ...
                            @foreach($trashedPosts as $comment)
                                <tr>
                                    
                                <td class="sorting_1 text-center">
                                            <label>
                                                <input type="checkbox" class="child-checkbox"  value="{{$comment->id}}" name="ids[]">
                                        </label>
                                </td>
                                    <td name="id" class="id">{{ $comment->id }}</td>
                                    <td name="title" id="title">{{ $comment->user->email }}</td>
                                    <td>{{ $comment->post->title }}</td>
                                    <td>
                                        <a href="{{ route('comment.restore', $comment->id) }}" class="btn btn-success">Khôi phục</a>
                                        <a href="{{ route('comment.forceDelete', ['id' => $comment->id]) }}" class="btn btn-danger
                                        ...
                                        <form action="{{ route('comment.forceDelete', $comment->id) }}" method="post" style="display:inline;" id="delete" >
                                            @csrf
                                        </form>
                    ...
                    if (selectedCheckboxes.length > 0) {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                ...
                                $.ajax({
                                    type: 'POST',
                                    url: '/admin/comment/permanentlyDeleteSelected', // Thay thế bằng tuyến đường xử lý xoá của bạn
                                    data: {
                                        ids: selectedIds,
                                        _token: '{{ csrf_token() }}',
                                    },
