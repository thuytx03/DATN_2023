        ...
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <div class="row align-items-center">
                    <div class="col">
                        <a href="{{ route('voucher.store') }}" class="btn btn-success">
                            Thêm mới
                        </a>
                    </div>
                    <div class="col text-right">
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu">
                                <a class="dropdown-item" href="{{ route('voucher.trash') }}">Thùng rác</a>
                                    ...
                                    <tbody>
                                        @foreach ($vouchers as $value)
                                            <tr>
                                                ...
                                                <td>{{ $value->value }}%</td>
                                                @else
                                                <td> {{ number_format($value->value, 0, ',', '.') }} VNĐ
                                                </td>
                                                ...
                                                @elseif($value->min_order_amount != '' && $value->max_order_amount == '')
                                                    <td>Đơn từ {{ number_format($value->min_order_amount, 0, ',', '.') }} VNĐ</td>
                                                @elseif($value->max_order_amount != '' && $value->min_order_amount == '')
                                                    <td>Đơn tối đa {{ number_format($value->max_order_amount, 0, ',', '.') }} VNĐ</td>
                                                @elseif($value->min_order_amount != '' && $value->max_order_amount != '')
                                                    <td> Đơn từ {{ number_format($value->min_order_amount, 0, ',', '.') }} VNĐ đến
                                                        {{ number_format($value->max_order_amount, 0, ',', '.') }} VNĐ</td>
                                                @endif
                                                <td>{{ $value->start_date }}
                                                </td>
                                                    ...
                                                    <div class="form-check form-switch">
                                                        ...
                                                        @elseif($value->status == 1 || $value->status == 2)
                                                            <input type="checkbox" value="{{ $value->status }}"
                                                                name="status" data-item-id="{{ $value->id }}"
                                                    ...
                                                    <div class="dropdown">
                                                        ...
                                                        <div class="dropdown-menu">
                                                            <a class="dropdown-item"
                                                                href="{{ route('voucher.update', ['id' => $value->id]) }}">Sửa</a>
                                                            <a class="dropdown-item show_confirm"
                                                                href="{{ route('voucher.destroy', ['id' => $value->id]) }}">Xoá
                                                            </a>
                        ...
                        <div class="row">
                            ...
                            ">
                                <div class="dataTables_info" id="dataTable_info" role="status" aria-live="polite">
                                    Hiển thị {{ $vouchers->firstItem() }} đến {{ $vouchers->lastItem() }}
                                    của {{ $vouchers->total() }} mục
                                </div>
                            </div>
                    ...
                    if (selectedCheckboxes.length > 0) {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                ...
                                $.ajax({
                                    type: 'POST',
                                    url: '/admin/voucher/deleteAll', // Thay thế bằng tuyến đường xử lý xoá của bạn
                                    data: {
                                        ids: selectedIds,
                                        _token: '{{ csrf_token() }}',
                                    },
